...
def one_time_raw_text(url, plain_text, method="GET"):
    """ one time request
    ...
    headers = {'User-Agent': user_agent}
    if method == "GET":
        response = requests.get(url, data=plain_text)
    else:
        response = requests.post(url, data=plain_text, headers=headers)
    print(response.text)
...
if __name__ == "__main__":
    ...
    one_time_raw_text('http://127.0.0.1:8889/rawtext', 'I love you\t1', 'POST')
